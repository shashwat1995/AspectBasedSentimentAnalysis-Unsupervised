...
for(j in 1:nrow(Dependency_final))
{
  ...
  {
    pola = pola +Dependency_final$polarity[j]
  }
}
Assomatrix$polarity[i]<-  pola
}
#############################################################
for(i in 1:nrow(Assomatrix))
{
  if(Assomatrix$polarity[i]>=0)
    Assomatrix$polarity[i] = 1
  else if(Assomatrix$polarity[i] <0)
    Assomatrix$polarity[i] = -1
}
...
## matched 1451 out of 1892 = 0.766
